...
public class OAuth2Util {
    ...
    public AuthProviderType getProviderTypeFromRegistrationId(String registrationId) {
        ...
        return switch (registrationId.toLowerCase()) {
            ...
            case "facebook" -> {
                log.info("OAuth2Util.getProviderTypeFromRegistrationId(): Provider resolved as FACEBOOK");
                yield AuthProviderType.FACEBOOK;
            }
            case "twitter" -> {
                log.info("OAuth2Util.getProviderTypeFromRegistrationId(): Provider resolved as TWITTER");
                yield AuthProviderType.TWITTER;
            }
            default -> {
                log.error("OAuth2Util.getProviderTypeFromRegistrationId(): Unsupported OAuth2 provider={}", registrationId);
    ...
    public String determineProviderIdFromOAuth2User(OAuth2User oAuth2User, String registrationId) {
        ...
        if (providerId == null || providerId.isEmpty()) {
            log.error("OAuth2Util.determineProviderIdFromOAuth2User(): providerId is NULL for provider={}", registrationId);
            throw new IllegalArgumentException("Unable to determine providerId for OAuth2 login");
        }

        // Log safe prefix (avoid logging entire providerId)
        log.info("OAuth2Util.determineProviderIdFromOAuth2User(): providerId extracted");
    ...
    public String determineUsernameFromOAuth2User(OAuth2User oAuth2User, String registrationId) {
        ...
        log.info("OAuth2Util.determineUsernameFromOAuth2User(): Started provider={}", registrationId);
        ...
        log.info("OAuth2Util.determineUsernameFromOAuth2User(): Extracted username={}", username);
        return username;
    }
}
