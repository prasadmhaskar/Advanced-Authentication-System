...
public class DeleteAccountService {
    ...
    public void deleteMyAccount(DeleteAccountRequest request, RequestContext ctx) {
        ...
        log.info("DeleteAccountService: started ip={}", ctx.ip());

        Long userId = authUtil.getCurrentUserId();
        String password = request.getPassword();

        log.info("DeleteAccountService: User with userId={}",userId);
        ...
        if (user.getPassword() != null) {
            if (password == null || !passwordEncoder.matches(password, user.getPassword())) {
                log.warn("DeleteAccountService: Deletion failed. Invalid password for user {}", userId);
        ...
        log.info("DeleteAccountService: finished ip={} User with userId={} deleted their own account successfully",ctx.ip(), userId);
    }
}
